...
const HomeSider = ({
  ...
  const menuItems = useMemo<MenuProps["items"]>(() => {
    const hasKey = (
      route: RouteConfig,
    ): route is RouteConfig & { key: string } => {
  ...
  return (
    <div className="flex flex-col h-full px-0 pt-4 pb-1 overflow-hidden">
      ...
      {isHomeSider ? (
        ...
        >
          <img
            src={logoUrl}
            alt="logo"
            className={clsx("h-8 w-auto", collapsed && "hidden")}
          />
        </div>
      ) : null}
      ...
      {collapsed ? null : (
        <div
          className="mx-3 my-2 h-px shrink-0 bg-[var(--dip-border-color)]"
          aria-hidden
        />
      )}
