...
const MicroAppComponent = ({ appBasicInfo, homeRoute, customProps }: MicroAppComponentProps) => {
  ...
  const microAppPropsRef = useRef<(Omit<MicroAppProps, 'container'> & Record<string, unknown>) | null>(null);
  ...
  const microAppProps = useMemo(
    () => ({
      ...
      onMicroAppStateChange: (callback: (state: any, prev: any) => void, fireImmediately?: boolean) => {
  ...
  if (loadFailed && failureInfo) {
    ...
    const errorMessage = failureInfo.error instanceof Error ? failureInfo.error.message : String(failureInfo.error);
