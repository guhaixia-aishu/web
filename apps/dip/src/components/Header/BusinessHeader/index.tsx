...
const BusinessHeader = () => {
  ...
  const businessHeaderBreadcrumbHidden = useGlobalLayoutStore(s => s.businessHeaderBreadcrumbHidden);
  const businessSiderHidden = useGlobalLayoutStore(s => s.businessSiderHidden);
  const businessHeaderCustomBreadcrumbLabel = useGlobalLayoutStore(s => s.businessHeaderCustomBreadcrumbLabel);
  ...
  const isInitialConfigOnlyMode = currentRoute?.key === 'initial-configuration' && breadcrumbMode === 'init-only';
  ...
  return (
    <>
      ...
      <div className="flex items-center gap-x-4 h-full">{showHeaderUserInfo ? <UserInfo /> : null}</div>
    </>
  );
};
