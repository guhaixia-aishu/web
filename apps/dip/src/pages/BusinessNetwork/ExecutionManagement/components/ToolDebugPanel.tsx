...
const ToolDebugPanel = ({ boxId, selectedTool }: Props) => {
  const [loading, setLoading] = useState(false);
  const [payloadText, setPayloadText] = useState('{}');
  const [result, setResult] = useState<any>(null);

  const toolId = selectedTool?.tool_id;
  const disabled = useMemo(() => !boxId || !toolId, [boxId, toolId]);
  ...
  const handleRun = async () => {
    ...
    setLoading(true);
    try {
      const data = await debugTool(boxId, toolId, payload);
      setResult(data);
  ...
  return (
    <div className="mt-4 border rounded p-4">
      ...
      <Typography.Paragraph type="secondary">
        从 `operator-web` 调试能力迁移的轻量版本：填写 JSON 入参后调用工具调试接口。
