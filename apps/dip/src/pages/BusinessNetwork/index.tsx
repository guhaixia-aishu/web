...
import { getFullPath, BASE_PATH } from "@/utils/config";
import {
...
const BusinessNetwork = () => {
  ...
  const currentMenu =
    businessLeafMenuItems.find((item) =>
      location.pathname.startsWith(item.path),
    ) ?? defaultBusinessMenuItem;
  const [microAppInfo, setMicroAppInfo] = useState<CurrentMicroAppInfo | null>(
    null,
  );
    ...
    return buildBusinessMicroAppProps({
      ...
      navigateToMicroWidget: (params) => {
        ...
        const item = businessLeafMenuItems.find(
          (menuItem) =>
            menuItem.page?.type === "micro-app" &&
            menuItem.page?.app?.name === params.name,
        );
        if (!item) return;
      ...
      changeCustomPathComponent: (param: { label: string } | null) => {
        ...
        useGlobalLayoutStore
          .getState()
          .setBusinessHeaderCustomBreadcrumbLabel(param?.label ?? null);
      },
    });
  ...
  const content = useMemo(() => {
    ...
    const ComponentPage =
      businessComponentPageRegistry[currentMenu.page.componentKey];
    if (ComponentPage) {
      return (
        <Suspense
          fallback={
  ...
  }, [currentMenu, customProps, microAppInfo]);
